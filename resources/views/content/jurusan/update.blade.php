    ...
    <div class="max-w-md mx-auto">
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <form action="{{ route('jurusanUpdate', $data['jurusan']->id) }}" method="post">
                @csrf
                <x-input name="namaJurusan" label="Nama Jurusan" :value="$data['jurusan']->Nama_jurusan" placeholder="Masukan nama Jurusan" :required="true" />
                <x-input name="kaproli" label="Nama Kaprodi" :value="$data['jurusan']->Nama_kaproli" placeholder="Masukan nama Kaprodi" :required="true" />
